				...
				<c:forEach var="ProductList" items="${ProductList}">
					<div class="col-md-3">
						...
						<div class="card ">
							...
							<div class="card-body">
								<h4 class="card-title">상품명: ${ProductList.name}</h4>
								<p class="card-text">제조사: ${ProductList.manufacturer}</p>
								<p class="card-text">가격: ${ProductList.price}원</p>
								<a
									href="<c:url value="/viewProduct?page=${page}&productid=${ProductList.productid}&category=null" />"
		...
		<div class="text-light">
			<c:if test="${prev}">
				<span>[ <a class="text-light"
					href="<c:url value="/products?page=${startPageNum - 1}"/>">이전</a> ]
				</span>
			</c:if>

			<c:forEach begin="${startPageNum}" end="${endPageNum}" var="page">
				<span> <a class="text-light"
					href="<c:url value="/products?page=${page}"/>">${page}</a>
				</span>
			</c:forEach>

			<c:if test="${next}">
				<span>[ <a class="text-light"
					href="<c:url value="/products?page=${endPageNum + 1}"/>">다음</a> ]
				</span>
			</c:if>
		</div>
	</div>
